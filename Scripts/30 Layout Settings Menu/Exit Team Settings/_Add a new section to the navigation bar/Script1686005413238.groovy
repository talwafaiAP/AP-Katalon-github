...
if (WebUI.executeJavaScript('return document.querySelector(\'body > app-root > main > loader > div > main-content > div.main-content.sidemenu-expanded > planning-views > div > div.data-container-wrapper-filter-overlay.data-container-wrapper-filter-collapsed > div.left.oauto.data-container.scrollable.data-container-filter-overlay.data-container-filter-collapsed > loader > div > div > pane-container > div.is-expanded\')',
	...
	for (def i = 1; i <= teamViewElementsCount; i++) {
		...
		if (teamViewText == teamViewName) {
			...
			String teamViewToDeleteQuery= 'return document.querySelector(' + jsQuerySelector1+ i + ') > planning-view-item > div > div.planning-view-item-buttons-container > div.planning-view-item-selected.fs14.icon-FavoriteNotSelected\''+ ').click();';
			println(teamViewToDeleteQuery);
			WebUI.executeJavaScript(teamViewToDeleteQuery,null)
			WebUI.delay(2)
			println(teamViewText + " was ADDED TO SECTION ______________________________________")			
...
WebUI.executeJavaScript('return document.querySelector(\'body > app-root > main > loader > div > side-bar > div > div.container-inner.scrollable > ul:nth-child(3) > div > section-label > div\').click();',null);

String teamInSection= WebUI.executeJavaScript('return (document.querySelector(\'body > app-root > main > loader > div > side-bar > div > div.container-inner.scrollable > ul:nth-child(3) > li > div > nav-link > a > span\')).innerHTML',null);
println(teamInSection)
if(teamInSection==teamViewName) {
	println(teamViewName + "   FOUND in SECTION ________________________")
}else {
	println('NOT Found in section')
}
